...
本流程适用于 GPT-5.6-terra（high）与 GPT-5.6-sol 的多会话开发。

## Skill 主链

1. `/grill-with-docs` 与 `/domain-modeling` 对齐需求、词汇和 ADR。
2. `/research` 仅调查需要一手资料确认的外部技术事实。
3. `/to-spec` 由 GPT 生成中文规格 issue，并先请用户确认测试 seam。
4. `/to-tickets` 由 GPT 拆成单会话可完成的纵向切片，声明 blocker、模型等级和风险。
5. 每张工单在独立 branch 与 worktree 中由新会话运行 `/implement`；内部尽量使用 `/tdd`。
6. 每张实现工单创建一个 PR，并由独立 GPT 会话运行 `/code-review`。
7. blockers-first 合并；下游审查前同步最新 `main` 并重跑测试。
8. 所有工单合并后，由独立 GPT 集成会话运行完整 CI、Docker 冒烟和规格回归。
...
负责规格、拆票、ADR、安全与凭据设计、跨模块接口、高风险实现、调度、审查、集成和真实环境诊断。GPT 调度会话维护 frontier，但不代替实现会话。

### GPT-5.6-terra（high）

只领取同时满足以下条件的工单：

- 标签为 `agent:terra` 和 `risk:low`。
- 所有 blocker 已关闭。
- 没有未决架构或安全决策。
- 验收标准、允许修改的模块边界和验证命令完整。

Terra 遇到 ADR 冲突、跨模块重构、安全选择、无法稳定复现的测试或需要扩大范围时，必须停止修改，在 issue 中记录命令、输出和已改文件，并改派 GPT-5.6-sol。
...
- Terra PR 必须由独立 GPT-5.6-sol 做 Standards 与 Spec 双轴审查。
- `risk:high` 的 GPT PR 也必须由另一个 GPT 会话审查。
