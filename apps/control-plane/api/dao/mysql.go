...
func InitMySQL() {
	...
	if count == 0 {
		if err = validateInitialSysadminPasswordFile(); err != nil {
			logrus.Errorf("initial sysadmin credential is unavailable: %v", err)
			panic(err)
		}
	...
	if err = initializePendingSysadminPassword(); err != nil {
		logrus.Errorf("initial sysadmin credential initialization failed: %v", err)
		panic(err)
	}
